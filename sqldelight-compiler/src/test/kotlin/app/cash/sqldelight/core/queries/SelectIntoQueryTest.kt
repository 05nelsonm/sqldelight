...
class SelectIntoQueryTest {
  ...
  @Test fun `embeddedSQL works with single select`() {
    ...
    assertThat(generator.customResultTypeFunction().toString()).isEqualTo(
      """
      |public fun select(): app.cash.sqldelight.ExecutableQuery<kotlin.Long> = app.cash.sqldelight.Query(-1626977671, driver, "Test.sq", "select", "SELECT abs(42)") { cursor ->
      |  cursor.getLong(0)!!
      |}
      |
      """.trimMargin(),
  ...
  @Test fun `embeddedSQL works with set select`() {
    ...
    assertThat(generator.customResultTypeFunction().toString()).isEqualTo(
      """
      |public fun setSelect(): app.cash.sqldelight.ExecutableQuery<kotlin.Long> = app.cash.sqldelight.Query(1705584993, driver, "Test.sq", "setSelect", "SELECT abs(42)") { cursor ->
      |  cursor.getLong(0)!!
      |}
      |
      """.trimMargin(),
